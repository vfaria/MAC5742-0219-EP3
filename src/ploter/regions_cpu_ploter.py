...
def main():
    ...
    for line in infile:
        pth_means.append(float(line.strip())/1000)
        omp_means.append(float(next(infile).strip())/1000)
        seq_means.append(float(next(infile).strip())/1000)
    ...
    ax.legend((rects1[0], rects2[0], rects3[0]), ('pth 32t', 'omp 32t', 'seq w/o io'))
